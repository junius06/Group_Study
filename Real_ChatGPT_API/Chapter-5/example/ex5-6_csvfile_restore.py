...
sim_result = return_answer_candidate(df, '삼시세끼')
print(sim_result)



## 기존 데이터프레임(df)를 csv파일로 저장하고, 필요시 호출하는 방식으로 진행할 수 있다.

# 데이터프레임(df)를 csv로 변환하여 저장하고, 생성된 파일은 명령어가 실행되는 경로에 저장된다.
df.to_csv('df_backup.csv', index=False, encoding='utf-8-sig')

# 추후 필요할 때 pandas의 pd.read_csv()를 이용해 csv파일을 데이터프레임으로 불러오면 이전과 같이 df를 생성하지 않고 기존의 df와 동일한 값을 가진 데이터프레임을 바로 사용할 수 있다.
